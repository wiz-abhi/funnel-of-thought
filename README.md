...
**Needs:** Docker, Python 3.11+, a free [Gemini key](https://aistudio.google.com/apikey). Windows: Git Bash, or the `.ps1` twin of each script.
...
pip install -e ".[all]"
./scripts/setup.sh                    # token + funnels + dashboard + alert + batch (~10 min)
...
Generates a two-model batch, builds both funnels, prints the working cliff, then triggers the fragmented funnel and shows the `500: unsupported value: NaN` verbatim. It exits non-zero if any asserted claim fails to reproduce — a green run means this README held on *your* machine.
...
- **A funnel over cognition, not commerce** — pointing Trace Funnels at an agent's *reasoning contract*, which nobody has made work end-to-end on a stock-instrumented agent.
